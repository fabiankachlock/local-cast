...
class SenderController {
  ...
  constructor() {
    Alpine.store("sender", {
      ...
      leave: () => this.leave(),
      changeAudio: (id: string) => this.changeAudio(id),
  ...
  public async join(roomId: string) {
    ...
    for (const track of this.stream.getTracks()) {
      await this.pc.addTrack(track, this.stream);
      console.log(track);
    }
  ...
  private async setupStream() {
    this.stream = await this._streamProvider.requestUserMedia({
      video: true,
      audio: true,
    });
  }
  ...
  public async changeAudio(id: string) {
    const stream = await this._streamProvider.applyConstraints({
